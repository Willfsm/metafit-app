...
import { ChevronRight, X, Plus, BrainCircuit, Info, Sparkles, Edit, Save, BookCopy, Timer, Target, RefreshCw, Calendar, Trash2, TrendingUp, Dumbbell, Check } from 'lucide-react';
...
const DUMMY_SCHEDULE = { 0: 'p1', 1: 'p2', 2: 'p1', 3: 'p2', 4: 'p1', 5: 'p2', 6: null }; // Dom, Seg, Ter, Qua, Qui, Sex, Sab
...
const Goals = ({ goals, setGoals }) => {
    ...
    return (
        <div className="bg-[#1E3A44] border border-gray-700 p-4 rounded-xl shadow-lg h-full flex flex-col">
            <h3 className="font-bold text-lg mb-3 flex items-center text-gray-100"><Target className="mr-2 text-[#CB6330]"/>Quadro de Metas</h3>
                ...
                {(goals[activeTab] || []).map(goal => (
                    <div key={goal.id} className="flex items-center p-1 rounded group">
                        ...
                        <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => startEditing(goal)} className="p-1 text-gray-400 hover:text-yellow-400"><Edit size={16}/></button>
                            <button onClick={() => deleteGoal(goal.id)} className="p-1 text-gray-400 hover:text-red-500"><Trash2 size={16}/></button>
                        </div>
                    </div>
                ))}
            </div>
            <div className="flex space-x-2 mt-4 border-t border-gray-700 pt-3">
                ...
                <button onClick={handleAddGoal} className="bg-[#CB6330] text-white font-bold p-2 rounded-lg hover:bg-[#D69369]"><Plus size={20}/></button>
            </div>
        </div>
    );
};
const QuickStats = ({ workouts }) => {
    ...
    return (<div className="bg-[#1E3A44] border border-gray-700 p-4 rounded-xl shadow-lg h-full"><h3 className="font-bold text-lg mb-4 flex items-center text-gray-100"><TrendingUp className="mr-2 text-[#D69369]"/>Estatísticas Rápidas</h3><div className="space-y-4">{[{icon: <Dumbbell/>, label: 'Total de Treinos', value: totalWorkouts}, {icon: <Calendar/>, label: 'Treinos (Últimos 7 dias)', value: workoutsThisWeek}, {icon: <Sparkles/>, label: 'Recorde no Supino', value: benchPressPr}].map(stat => (<div key={stat.label} className="flex items-center justify-between"><div className="flex items-center text-gray-300"><span className="mr-3 text-[#D69369]">{stat.icon}</span>{stat.label}</div><div className="font-bold text-lg text-[#E8C69B]">{stat.value}</div></div>))}</div></div>);
};
        ...
        <div className="container mx-auto p-4 md:p-8 max-w-7xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <button className="w-full bg-gradient-to-r from-[#D69369] to-[#CB6330] text-white font-bold py-4 px-6 rounded-xl shadow-lg hover:opacity-90 transition-all flex items-center justify-center space-x-2" onClick={handleStartNewWorkout}>
                    <Plus size={20} /><span>Registar Treino do Dia</span>
                </button>
            ...
            <Modal visible={addWorkoutModalVisible} onClose={resetAndCloseModal} size="2xl">
                ...
                <div className="p-4 bg-[#13272E] border-t border-gray-700 flex space-x-3">
                    <button onClick={handleSaveWorkout} className="flex-1 bg-gradient-to-r from-[#D69369] to-[#CB6330] text-white font-bold py-3 rounded-lg hover:opacity-90 flex items-center justify-center">
                        <Save className="mr-2" size={18}/>Salvar Treino
                    </button>
                </div>
            </Modal>
